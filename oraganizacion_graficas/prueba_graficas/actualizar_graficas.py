...
while True:
    try:
        ...
        for node_id in nodos:
            ...
            # **Diferenciar entre nodos interiores y exteriores**
            ext_value = data_node["ext"].iloc[0]  # Se asume que el valor es consistente en cada nodo
            tipo_nodo = "interior" if ext_value == 0 else "exterior"

            # **Definir ruta de almacenamiento**
            node_dir = os.path.join(MAT_DIR, f"nodo_{tipo_nodo}_{node_id}")
            png_dir = os.path.join(PNG_DIR, f"nodo_{tipo_nodo}_{node_id}")
            ...
            mat_filename = os.path.join(node_dir, f"nodo_{tipo_nodo}_{node_id}.mat")
            ...
            for key, (titulo, unidad, color, archivo) in medidas.items():
                ...
                # **Graficar**
                plt.plot(ordenado["timestamp"], ordenado[key], color, marker="o", label=f"{titulo} del Nodo {node_id} ({tipo_nodo})")

                plt.xlabel("Hora del día (HH:MM)")
                plt.ylabel(f"{titulo} ({unidad})")
                plt.title(f"{titulo} del Nodo {node_id} ({tipo_nodo})")
                plt.grid(True)
                plt.legend()
                plt.tight_layout()
